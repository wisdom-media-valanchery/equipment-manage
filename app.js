...
function renderFundsTab() {
    ...
    sortedAdditions.forEach(f => {
        ...
        additionsBody.innerHTML += `
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3 text-sm font-bold text-green-600 text-right">₹${f.amount.toLocaleString('en-IN', {maximumFractionDigits: 2})}</td>
    ...
    sortedExpenses.forEach(exp => {
        ...
        usageBody.innerHTML += `
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3 text-sm font-bold text-red-600 text-right">₹${price.toLocaleString('en-IN', {maximumFractionDigits: 2})}</td>
                <td class="px-4 py-3 text-sm text-center admin-only ${(currentUserRole === 'admin' || currentUserRole === 'fund') ? '' : 'hidden'}">
    ...
    document.getElementById('fund-total-collected').textContent = '₹' + totalCollected.toLocaleString('en-IN', {maximumFractionDigits: 2});
    document.getElementById('fund-total-spent').textContent = '₹' + totalSpent.toLocaleString('en-IN', {maximumFractionDigits: 2});
    
    const balanceEl = document.getElementById('fund-available-balance');
    balanceEl.textContent = '₹' + available.toLocaleString('en-IN', {maximumFractionDigits: 2});
    if (available < 0) {
